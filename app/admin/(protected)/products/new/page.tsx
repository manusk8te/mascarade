...
export default function NewProductPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      // Upload photos first via server API (uses service role key)
      const variantsWithUrls = await Promise.all(
        variants.map(async (v, i) => {
          ...
          if (v.photo_file) {
            ...
            if (!res.ok) setError(`Upload échoué : ${json.error}`)
            photo_url = json.url || null
          }
          return { ...v, color_name: colorName, photo_url }
        })
      )

      // Create product + variants via server API
